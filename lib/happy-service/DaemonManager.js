...
function getLocalHappyPath() {
    ...
    try {
        ...
        if (app && app.isReady && app.isReady()) {
            ...
            if (!app.isPackaged) {
                ...
                // 打包环境：appPath 是 app.asar，lib 在 app.asar/lib
                // 从 lib/happy-service 向上找：.. -> lib, .. -> app.asar
    ...
    // 优先使用 node_modules 中的 happy-coder（有完整依赖）
    // 在打包后，node_modules 可能在 app.asar/node_modules 或 resources/node_modules
    const possiblePaths = [
        path.join(appRoot, 'node_modules', 'happy-coder', 'bin', 'happy.mjs'),
        path.join(__dirname, '..', '..', 'node_modules', 'happy-coder', 'bin', 'happy.mjs'), // Fallback
    ];
    ...
    // 回退：使用 lib/happy-cli（在 asar 中应该可以访问）
    const libPath = path.join(__dirname, '..', 'happy-cli', 'bin', 'happy.mjs');
    if (fs.existsSync(libPath)) {
        return libPath;
    }
    
    // 最后的回退：使用 appRoot 下的 lib/happy-cli
...
class DaemonManager {
    ...
    async _doStartDaemon(timeout) {
        return new Promise((resolve, reject) => {
            ...
            // 注意：不能用 process.execPath，因为在 Electron 环境中它指向 Electron 可执行文件
            // Electron 内置的 Node.js 版本可能较旧，不支持 fastify 所需的 diagnostics.tracingChannel API
            const daemonProcess = spawn('node', [happyBinPath, 'daemon', 'start'], {
                detached: true,
                stdio: ['ignore', 'ignore', 'pipe'], // 捕获 stderr
                shell: true, // 使用 shell 以便正确解析 'node' 命令
                windowsHide: true,
                env: daemonEnv
            });
            ...
            const checkInterval = setInterval(() => {
                ...
                if (this.isDaemonRunning()) {
                    clearInterval(checkInterval);
                    const state = this.getDaemonState();
                    logInfo(`Daemon started (PID: ${state?.pid}, Port: ${state?.httpPort})`);
                    resolve(true);
                } else if (Date.now() - startTime > timeout) {
                    clearInterval(checkInterval);
                    let errorMsg = 'Daemon start timeout';
                    if (stderrOutput) {
