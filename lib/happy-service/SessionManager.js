...
class SessionManager extends EventEmitter {
    ...
    async createSessionForWorkDir(workDir) {
        ...
        if (existingName && this.sessions[existingName]) {
            ...
            // session 不再有效，清理旧记录
            logInfo(`Old session ${existingSession.sessionId} no longer valid, creating new one`);
        }
        ...
        // 4. 更新目录索引
        const normalizedPath = this._normalizePathForIndex(resolvedPath);
        this.workDirIndex[normalizedPath] = sessionName;
    ...
    saveStateFile() {
        const state = {
            ...
            workDirIndex: this.workDirIndex
        };
    ...
    loadStateFile(currentAnonId = null) {
        ...
        try {
            ...
            this.workDirIndex = state.workDirIndex || {};
            
            // 兼容旧格式：如果没有 workDirIndex，从 sessions 重建
            if (Object.keys(this.workDirIndex).length === 0 && Object.keys(this.sessions).length > 0) {
    ...
    async reconnectSession(name) {
        const session = this.sessions[name];
        if (!session) {
            logWarn(`Session "${name}" does not exist`);
            return false;
        }
        
        logInfo(`Reconnecting session "${name}"...`);
        
        try {
            const result = await this.daemonRequest('/spawn-session', { directory: session.workDir });
            ...
            // 尝试获取新 PID
            const sessionEntry = await this.findSessionById(result.sessionId);
            if (sessionEntry) {
                session.pid = sessionEntry.pid;
            }
            
            logInfo(`Session "${name}" reconnected: ${result.sessionId}`);
            this.emit('session:reconnected', { name, sessionId: result.sessionId });
            
            this.saveStateFile();
            return true;
        } catch (error) {
            logError(`Session "${name}" reconnect failed: ${error.message}`);
    ...
    async _monitorLoop() {
        ...
        const statuses = await this.checkAllSessionsStatus();
        
        for (const [name, status] of Object.entries(statuses)) {
            if (status === 'disconnected' || status === 'error') {
                logInfo(`Session "${name}" status abnormal (${status}), reconnecting...`);
                await this.reconnectSession(name);
                // 等待一段时间再处理下一个
                await sleep(2000);
            }
        }
    ...
    clearSessions() {
        this.sessions = {};
        this.stateCreatedAt = null;
        logInfo('Memory session state cleared');
    }
