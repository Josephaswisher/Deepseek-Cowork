...
class ConversationManager extends EventEmitter {
    ...
    try {
      ...
      if (content?.type === 'event') {
        ...
        if (eventType === 'ready') {
          ...
          // 外部 ready 事件不额外发出状态更新，避免干扰当前对话状态
          // （与 Fork/happy 原版行为保持一致）
          if (!isExternal) {
  ...
  _handleConversationMessage(conversation, text, decrypted) {
    ...
    if (isComplete) {
      ...
      // 静默检测定时器：作为备用机制，当 Agent Ready 事件未到达时
      // 在 3 秒内没有新消息时，自动发出 ready 状态
      if (!conversation.waitingForSkill) {
        conversation.silenceCheckerId = setTimeout(() => {
          if (conversation.status === 'active' && !conversation.waitingForSkill) {
            if (this.options.debug) {
              console.log(`[ConversationManager] Silence timeout, emitting ready`);
            }
            this._emitEventStatus('ready', { conversationId: conversation.id, source: 'silence_timeout' });
          }
        }, 3000);
      }
    }
  ...
  _handleAgentReady() {
    ...
    // 确保对话有消息（与 Fork/happy 原版行为保持一致）
    if (conversation.messages.length === 0) {
      if (this.options.debug) {
        console.log(`[Agent Ready] Conversation ${conversation.id} has no messages, skipping`);
      }
      return;
    }
