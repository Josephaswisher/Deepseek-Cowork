...
class LocalService extends EventEmitter {
    ...
    /**
     * 确保 Happy 凭证已同步到 ~/.happy/access.key
    ...
    async _ensureHappyCredentialsSynced(happySecret, serverUrl) {
        ...
        // 如果 access.key 已存在，无需同步
        if (fs.existsSync(accessKeyPath)) {
            return;
        }
        
        logInfo('access.key not found, syncing credentials to ~/.happy/...');
        ...
        try {
            ...
            try {
                ...
                // 即使获取 token 失败，也尝试写入 access.key（没有 token）
            }
            ...
            logInfo('Credentials synced to ~/.happy/access.key');
        } catch (error) {
