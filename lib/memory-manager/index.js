/**
 * MemoryManager - 对话记忆管理器（简化版）
 * 
 * 简化后的记忆管理：
 * - 始终注入所有活动记忆的 summary 合集
...
class MemoryManager extends EventEmitter {
     ...
     * @param {string} options.workDir 工作目录路径
     * @param {string} options.dataDir 数据目录路径（可选，默认从 workDir 推导）
     * @param {string} options.skillDir 技能目录路径（可选，默认从 workDir 推导）
     * @param {number} options.contextLimit 上下文总大小限制（tokens）
     * @param {Object} options.autoSave 自动保存配置
     */
    constructor(options = {}) {
        super();
        ...
        // 注意：不能直接 ...options，会覆盖已合并的 autoSave
        this.options = {
            workDir,
            contextLimit: options.contextLimit || 200000,
            autoSave: { ...config.autoSave, ...(options.autoSave || {}) }
        };
        
        // 两个核心目录
        // 数据目录：存储记忆文件（active/, archive/, index.md）
        this.dataDir = options.dataDir || path.join(workDir, config.paths.dataDir);
        // 技能目录：存储技能代码和脚本（SKILL.md, scripts/）
        this.skillDir = options.skillDir || path.join(workDir, config.paths.skillDir);
        ...
        this.scriptsDir = path.join(this.skillDir, config.paths.scriptsDir);
        ...
        this._contextLimit = this.options.contextLimit;
    ...
    async initialize() {
        ...
        try {
            ...
            console.log(`[MemoryManager] Skill dir: ${this.skillDir}`);
...
`;
    ...
    /**
     * 获取要注入到上下文的内容
     * 简化版：直接返回 index.md 的内容
     * @param {string} userMessage 用户消息（保留参数兼容性，但不再使用）
     * @returns {string} 要注入的上下文文本
     */
    getContextInjection(userMessage) {
        if (!this._initialized || !this._enabled) {
            return '';
        }
        
        if (!this._indexContent) {
            return '';
        }
        
        // 直接返回索引内容，让 AI 自行判断相关性
        return '\n\n---\n' + this._indexContent + '\n---\n\n';
    }
    ...
    updateBudget(usage) {
        ...
        this._contextUsage = usage.contextSize || 0;
        
        // 检查是否需要触发自动保存
        if (this.options.autoSave.onContextThreshold) {
            const usageRatio = this._contextUsage / this._contextLimit;
            if (usageRatio >= this.options.autoSave.onContextThreshold) {
                console.log(`[MemoryManager] Context threshold reached (${(usageRatio * 100).toFixed(1)}%), triggering auto-save`);
                this.emit('autoSave:threshold', { usageRatio });
            }
        }
    }
    ...
    /**
     * 记录消息（已废弃）
     * @deprecated 消息现由 MessageStore 管理，保存时从外部传入
     * @param {Object} message 消息对象
     */
    recordMessage(message) {
        // 空实现，保留方法签名以兼容旧代码
        console.warn('[MemoryManager] recordMessage() is deprecated, messages are now passed to saveConversation()');
    }
     ...
     * @param {string[]} options.decisions 关键决策（可选）
    ...
    async saveConversation(messages, options = {}) {
        ...
        if (!messages || messages.length < this.options.autoSave.minConversationLength) {
        ...
        try {
            ...
            const summary = this._generateSummary(messages, topic, keywords, options.aiSummary, options.decisions, options.conversationId, options.sessionId);
    ...
    /**
     * 保存当前对话为记忆（已废弃）
     * @deprecated 请使用 saveConversation(messages, options) 代替
     */
    async saveCurrentConversation(options = {}) {
        console.warn('[MemoryManager] saveCurrentConversation() is deprecated, use saveConversation(messages, options) instead');
        return { success: false, reason: 'Deprecated, use saveConversation() instead' };
    }
     ...
     * @param {string[]} decisions 关键决策（可选，已废弃，建议直接在 aiSummary 中包含）
     * @param {string} conversationId 对话ID（可选）
     * @param {string} sessionId Session ID（可选）
     * @private
     */
    _generateSummary(messages, topic, keywords, aiSummary, decisions = null, conversationId = null, sessionId = null) {
...
`;
    ...
    /**
     * 清空当前对话记录（已废弃）
     * @deprecated 消息现由 MessageStore 管理，无需在 MemoryManager 中清空
     */
    clearConversation() {
        // 空实现，保留方法签名以兼容旧代码
        console.warn('[MemoryManager] clearConversation() is deprecated, messages are now managed by MessageStore');
    }
